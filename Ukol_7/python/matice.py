...
def soucet_matic(matice1: list[list[int]], matice2: list[list[int]]) -> list[list[int]] | None:
    ...
    if not matice1 or not matice2:
        return None  # Pokud je některá matice prázdná, vracíme None
    if len(matice1) != len(matice2) or len(matice1[0]) != len(matice2[0]):
        print("Chyba: Matice mají různé rozměry a nelze je sečíst.")
        return None
...
def nasobeni_matic(matice1: list[list[int]], matice2: list[list[int]]) -> list[list[int]] | None:
    ...
    if len(matice1[0]) != len(matice2):
        print("Chyba: Počet sloupců první matice se neshoduje s počtem řádků druhé matice.")
        return None
...
def transpozice_matice(matice: list[list[int]]) -> list[list[int]]:
    ...
    if not matice:
        return []  # Pokud je matice prázdná, vracíme prázdnou matici
