...
def soucet_matic(matice1: list[list[int]], matice2: list[list[int]]) -> list[list[int]] | None:
    ...
    if len(matice1) != len(matice2) or (matice1 and len(matice1[0]) != len(matice2[0])):
        return None
    return [
...
def transpozice_matice(matice: list[list[int]]) -> list[list[int]]:
    ...
    return [[matice[j][i] for j in range(len(matice))] for i in range(len(matice[0]))] if matice else []

if __name__ == "__main__":
    matice1 = vytvor_matici(3, 2)
    matice2 = vytvor_matici(2, 3)
    ...
    print("Součet matic:")
    print(reprezentace_matice(soucet) if soucet else "Nelze sečíst matice.")

    nasobek = nasobeni_matic(matice1, matice2)
    print("Násobení matic:")
    print(reprezentace_matice(nasobek) if nasobek else "Nelze násobit matice.")
