...
def nasobeni_matic(matice1: list[list[int]], matice2: list[list[int]]) -> list[list[int]]:
    ...
    if len(matice1[0]) != len(matice2):
        print("Chyba: Počet sloupců v první matici se nerovná počtu řádků v druhé matici.")
        return None
...
if __name__ == "__main__":
    ...
    print("Matice 3:")
    print(reprezentace_matice(matice3))

    soucet = soucet_matic(matice1, matice3)
    print("Součet matic 1 a 3:")
    print(reprezentace_matice(soucet))

    nasobek = nasobeni_matic(matice1, matice2)
    print("Násobení matic 1 a 2:")
    print(reprezentace_matice(nasobek))

    transponovana = transpozice_matice(matice1)
    print("Transponovaná matice 1:")
